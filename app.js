(() => {
  ...
  // ✅ extractor robusto: toma TODOS los números de 3+ dígitos de una celda
  function extractLegajos(value) {
    const matches = String(value ?? "").match(/\d{3,}/g);
    return matches ? matches : [];
  }

  function uniqueSortLegajos(arr) {
  ...
  // =========================
  // ✅ AGRUPAR COLUMNAS POR DIA (por headers mergeados/vacíos)
  // - Una fecha inicia un grupo.
  // - Columnas siguientes con header vacío (o sin fecha) se asignan al mismo grupo
  //   hasta la próxima fecha.
  // =========================
  function buildDateGroups(headerRow, metricCol0) {
    const groups = [];
    let current = null;

    for (let c = 0; c < headerRow.length; c++) {
      if (c === metricCol0) continue;

      const label = headerRow[c];
      const d = parseDate(label);

      if (d) {
        // nueva fecha -> nuevo grupo
        current = { date: d, primaryCol: c, cols: [c] };
        groups.push(current);
      } else {
        // sin fecha: si hay grupo activo, esta columna pertenece a ese día
        // (esto resuelve headers mergeados o vacíos)
        if (current) current.cols.push(c);
      }
    }
  ...
  // =========================
  // ✅ LEGAJOS DESDE A16:AC25 PERO ESCANEANDO TODAS LAS COLS DEL GRUPO DEL DIA
  // =========================
  function getLegajosFromRangeForGroup(matrix, colIndices) {
  ...
  function buildCards(matrix, headerRow1, metricCol1) {
    ...
    const metricCol0 = metricCol1 - 1;

    const header = matrix[h];
    if (!header) throw new Error("Fila de fechas inválida (headerRow).");

    // ✅ en vez de columnas sueltas, grupos por día
    const dateGroups = buildDateGroups(header, metricCol0);
    if (!dateGroups.length) throw new Error("No se detectaron fechas en el header.");
    ...
    return dateGroups.map(g => {
      ...
      // para métricas usamos la columna principal del grupo (la que tiene la fecha)
      const c = g.primaryCol;

      for (let i = 0; i < rows.length; i++) {
        const row = rows[i] || [];
        const name = String(row[metricCol0] ?? "").trim();
      ...
      // ✅ legajos: escanear A16:AC25 en TODAS las columnas del grupo del día
      const legajos = getLegajosFromRangeForGroup(matrix, g.cols);

      return { date: g.date, lineaTM, lineaTT, legajos, table };
    });
  }
